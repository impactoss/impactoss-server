...
if Features.enabled?(:indicators)
  RSpec.describe RecommendationIndicatorsController, type: :controller do
    ...
    describe "Post create" do
      ...
      context "when not signed in" do
        it "does not allow creating a recommendation_indicator" do
      ...
      context "when signed in" do
        ...
        it "does not allow a guest (no roles) to create a recommendation_indicator" do
          sign_in guest
          expect(subject).to be_forbidden
        end

        allowed_create_roles.each do |role|
          it "allows #{role} to create a recommendation_indicator" do
        ...
        forbidden_create_roles.each do |role|
          it "does not allow #{role} to create a recommendation_indicator" do
            user = FactoryBot.create(:user, role.to_sym)
            sign_in user
            expect(subject).to be_forbidden
          end
        ...
        it "returns an error if params are incorrect" do
          ...
          post :create, format: :json, params: {
            recommendation_indicator: {description: "desc only", taxonomy_id: 999}
          }
        ...
        it "records what user created the recommendation_indicator", versioning: true do
    ...
    describe "Delete destroy" do
      ...
      context "when not signed in" do
        it "does not allow deleting a recommendation_indicator" do
          expect(subject).to be_unauthorized
        end
      end
      ...
      context "when user signed in" do
        ...
        it "does not allow a guest (no roles) to delete a recommendation_indicator" do
          sign_in guest
          expect(subject).to be_forbidden
        end

        allowed_destroy_roles.each do |role|
          it "allows #{role} to delete a recommendation_indicator" do
            user = FactoryBot.create(:user, role.to_sym)
            sign_in user
        ...
        forbidden_destroy_roles.each do |role|
          it "does not allow #{role} to delete a recommendation_indicator" do
            user = FactoryBot.create(:user, role.to_sym)
            sign_in user
            expect(subject).to be_forbidden
          end
